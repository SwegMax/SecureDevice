...
Java_com_example_securedevice_NativeBridge_isLibraryTampered(JNIEnv* env, jobject, jstring libraryPath) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "Native", "Expected: %ld, Current: %ld", expected_checksum, current_checksum);
...
Java_com_example_securedevice_NativeBridge_isMemoryInjected(JNIEnv* env, jobject) {
    ...
    bool result = false;
    __android_log_print(ANDROID_LOG_INFO, "Native", "Attempting to read from protected memory address...");
